...
- Dream Journal 会显示当前用户可见的全部梦境记录，并按日期自动分组为今天、昨天、本周更早、本月更早和更早记录。
- Dream Journal 支持本地实时搜索，搜索范围包括自动标题、原文、梦境摘要、情绪和意象。
- Dream Journal 支持“全部”“快速解析”“深度解析”“待同步”四个过滤入口。
...
本轮没有实现 Timeline、Calendar、Favorite、Trash、Edit、Delete、Growth、Atlas、支付、会员或新的详情系统。“待同步”只是显示既有待同步状态；搜索和过滤都在当前页面本地完成。
# 下一步可以做什么
适合初学者继续添加的功能：
