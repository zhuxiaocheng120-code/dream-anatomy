...
详情页中的荣格、弗洛伊德和现代心理学折叠卡片只根据已有记录内容做温和展示，不提供诊断、治疗、算命、吉凶判断或未来预测。

## Dream Result Card 的当前边界

Dream Detail 可以读取现有记录中的 `reportContent.dreamResultCard`，也可以由用户手动为当前记录生成一次 **梦境画像**。生成时复用现有后端 DeepSeek 代理，并把结果保存到现有记录内容里，不新增 schema 或数据库字段。

当前只提供页面内的分享卡片预览，不会自动为所有历史记录生成画像；图片下载和分享功能未实现。
