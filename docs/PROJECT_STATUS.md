...
- 原生微信小程序基础工程已加入 `miniprogram/`，支持快速解析、梦境画像、本机保存、本机梦境日记、详情、删除、导出、清除本机数据，以及通过 Render 建立安全的微信身份 Session；当前仍不接 Supabase 登录、云同步、支付、会员或小程序产品行为分析事件。小程序视觉语言已同步 Web 端的旧纸、私人档案、心理工作室和手稿记录方向。
- Web Beta 已加入原创 Dream Guide 品牌标识，包括云朵梦境向导 Icon、横向 Lockup、单色 SVG、favicon 引用和克制的 CSS 微动画；详细边界见 `docs/BRAND_ASSETS.md`。
...
miniprogram/：微信原生小程序基础工程，包含页面、组件、本机存储、法律同意适配、快速解析请求、微信身份桥接和梦境画像展示。
...
隐私与数据控制需要在 Supabase SQL Editor 手动执行 `supabase/migrations/20260717001000_create_legal_consents.sql` 和 `supabase/migrations/20260721000000_add_cross_border_legal_consent.sql`，并在 Render 配置公开的 `PUBLIC_OPERATOR_NAME=朱校成` 与 `PUBLIC_SUPPORT_EMAIL=zhuxiaocheng120@gmail.com`。详细步骤见 `docs/PRIVACY_DATA_CONTROLS_SETUP.md` 和 `docs/LEGAL_AND_PRIVACY_SETUP.md`。

## 隐私与数据控制的当前边界

当前法律文件版本统一为 `2026-07-21`，用户可见文案适用于 Dream Anatomy 当前公开测试版本。当前确认的运营者为朱校成，主体类型为个人运营者，公开联系邮箱为 `zhuxiaocheng120@gmail.com`。Render Web/API 部署地区为美国俄勒冈州（Oregon, US West），Supabase 项目地区为印度孟买（South Asia / Mumbai，ap-south-1），AI 服务提供方为 DeepSeek API。
...
导出现在分为主要的可阅读 HTML 梦境档案 `dream-anatomy-archive-YYYY-MM-DD.html` 和次要的原始 JSON 备份 `dream-anatomy-export-YYYY-MM-DD.json`。导出只包含当前用户或当前浏览器游客可见的梦境记录和法律版本信息，不包含 token、邮箱、完整 Supabase UUID、principal_hash、微信身份 hash、管理员统计或其他账户数据。

正式面向中国境内公众扩大提供生成式 AI 功能前，需要确认所调用模型备案或登记信息、Dream Anatomy 应用或功能是否需要属地登记，以及是否需要在产品显著位置展示模型名称、备案号或上线编号。当前代码不会编造或展示假的备案号。

## 微信小程序的当前边界

小程序当前是 Web Beta 后续移动端体验的原生基础工程，已实现游客核心闭环和安全微信身份桥接。快速解析通过 `wx.request` 调用现有 Render 后端 `POST /api/v1/dream-analysis`，请求体包含 `analysisType: "quick"` 和 `clientPlatform: "wechat_mini_program"`，不发送微信身份认证头，也不直接调用 DeepSeek。
...
当前已接入 Render 后端微信身份桥接：用户可在“我的”页面主动点击“使用微信身份继续”，服务端通过微信 code 验证后返回 Dream Anatomy 不透明 Session Token。当前不做 code2Session 小程序直连、不保存原始 openid / unionid / session_key、不接 Supabase 登录、云同步、支付、会员、小程序产品分析事件或深度引导。深度引导入口保留展示，并标记为“正在开发中”。小程序设置和验收步骤见 `docs/MINIPROGRAM_SETUP.md`，架构边界见 `docs/MINIPROGRAM_ARCHITECTURE.md`，微信身份部署见 `docs/WECHAT_AUTH_SETUP.md`。真机验收尚未完成。
