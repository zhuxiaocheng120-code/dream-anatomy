...
- 快速解析：梦境记录工作台和手稿输入区。
- 结果页：心理档案报告感，区分文字解析、梦境画像和保存操作。
- 梦境日记：私人梦境档案和索引卡列表。
- 梦境详情：手稿记录与分析报告结合，长文本可换行。
...
1. 首页、快速解析、结果页、梦境日记、梦境详情、隐私与数据、我的页面都能打开。
2. 小屏幕模拟器下无横向溢出。
3. 输入长梦境时快速解析页面仍可操作。
4. 长 AI 结果、Dream Result Card 和导出 JSON 不破坏布局。
5. 删除和清空确认弹窗仍清楚可辨。
6. 深度引导仍显示“正在开发中”，不能触发 API。
